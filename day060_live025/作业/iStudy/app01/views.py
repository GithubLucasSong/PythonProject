...
from app01.forms import RegFrom,ArticleForm,ArticleDetailForm,CategoryForm
import hashlib
...
def get_query(request,fidld_list):
    # 传入一个字段列表，返回一个Q对象
    query = request.GET.get('query','')
    q = Q()
    q.connector = 'OR'
    for field in fidld_list:
...
def article_list(request):
    ...
    article_all = models.Article.objects.filter(q,author=request.user_obj)
    page = Pagination(request,article_all.count(),2)
    article_all = article_all[page.start:page.end]
    page_html = page.page_html
...
def category_list(request):
    ...
    return render(request,'category_list.html',{'category_all':category_all})
...
def category_change(request,pk=None):
    ...
    return render(request,'category_change.html',{'form_obj':form_obj,'title':title})
